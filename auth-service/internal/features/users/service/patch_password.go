...
func (s *usersService) PatchPassword(ctx context.Context, userID int, patchPasswordCommand core_domain.PatchPasswordCommand) error {
	...
	if err = s.transactionManager.WithTransaction(ctx, func(txCtx context.Context) error {
		...
		if err := s.sessionsService.RevokeSessions(txCtx, userID); err != nil {
